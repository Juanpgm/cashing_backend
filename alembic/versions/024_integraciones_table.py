...
Revises: 034_cuenta_cobro_consecutivo_ds
Create Date: 2026-07-28

Note: appended at the tail of the chain (down_revision repointed from the
original 023_documento_requisito_vinculos to 034_cuenta_cobro_consecutivo_ds,
the current tracked master head) during reintegration of this change onto
master — master had advanced 119 commits (through migration 034) since this
slice was originally authored, and 024 was never actually applied to any
shared DB in that time, so re-splicing it at the tail avoids a forked head
instead of renumbering every migration in between. (035_uq_contrato_numero_cuota.py
is untracked/uncommitted local WIP unrelated to this change — not a real
predecessor.)
"""
...
down_revision = "034_cuenta_cobro_consecutivo_ds"
branch_labels = None
depends_on = None
