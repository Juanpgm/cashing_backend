...
- [x] 1.16 **SUPERSEDED (confirmed 2026-07-25):** the planned standalone PR #1 was never opened. Commits landed instead on `integracion-stepper-local`, consolidated into backend PR #4 (open, not merged): https://github.com/Juanpgm/cashing_backend/pull/4.
...
- [x] 2.12 **SUPERSEDED (confirmed 2026-07-25):** same as 1.16 — consolidated into backend PR #4 (open, not merged): https://github.com/Juanpgm/cashing_backend/pull/4.
...
- [x] 4.8 **SUPERSEDED (confirmed 2026-07-25):** same as 1.16 — consolidated into backend PR #4 (open, not merged): https://github.com/Juanpgm/cashing_backend/pull/4.

  Commits `1973af9` (feat: DocumentId index + decimal-comma fix), `5e4e000` (feat: scraper persistence, fallback path), `4ba5f43` (docs: testing-swagger.md + tasks.md) are pushed and present on `origin/integracion-stepper-local` (verified via `git merge-base --is-ancestor`). No standalone chained PRs (#1-#4 as originally planned) were opened — all four slices ship together in PR #4, title "feat(stepper): radicacion stepper backend integration, context-grounded generation, SECOP acquisition". `secop-full-acquisition` code/tests are complete; the change now waits on PR #4 review/merge, not on further pushes. Once #4 merges, this change can move to `sdd-verify`/`sdd-archive`.
