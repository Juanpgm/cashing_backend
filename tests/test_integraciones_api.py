...
def _enable_ms365_for_tests(monkeypatch: pytest.MonkeyPatch) -> None:
    """MS365_INTEGRATION_ENABLED defaults to False (coded-but-not-active HTTP gate,
    see `app.api.v1.integraciones._require_ms365_enabled`). This module exists
    specifically to exercise the `/microsoft/*` HTTP surface, so it opts in for
    every test here; `TestMs365Disabled` below flips it back off to cover the
    default-disabled behavior itself.
    """
    monkeypatch.setattr(settings, "MS365_INTEGRATION_ENABLED", True)


class TestMs365Disabled:
    @pytest.mark.asyncio
    async def test_microsoft_routes_404_when_flag_disabled(
        self, client, test_user, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Default (flag off) behavior: every `/microsoft/*` route 404s; `/google/*` is unaffected."""
        monkeypatch.setattr(settings, "MS365_INTEGRATION_ENABLED", False)
